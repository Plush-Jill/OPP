...
struct task_queue_t *task_queue_create(int capacity) {
    struct task_queue_t *queue = static_cast<task_queue_t *>(malloc(sizeof(struct task_queue_t)));
    if (queue == NULL) {
        return NULL;
    }

    struct task_t *data = static_cast<task_t *>(malloc(sizeof(struct task_t) * capacity));
    if (data == NULL) {
        return NULL;
    }
