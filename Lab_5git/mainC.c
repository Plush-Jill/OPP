...
#define TASK_COUNT              20//10000
#define TOTAL_SUM_WEIGHT        1000//47000000
...
static inline void execute_tasks() {
    while (true) {
        ...
        fprintf(stdout, "Worker %d doing task [ID: %d, pID: %d, W: %d]\n",
                process_id, task.id, task.process_id, task.weight);
        pthread_mutex_unlock(&mutex);

        for (int i = 0; i < task.weight; ++i) {
            for (int j = 0; j < 18000 * 50; ++j) {
    ...
    pthread_mutex_lock(&mutex);
    fprintf(stdout, "Worker %d finished current tasks\n",
            process_id);
    pthread_mutex_unlock(&mutex);
}
...
void *worker_start() {
    init_tasks();
    fprintf(stdout, "Worker %d has %d tasks\n",
            process_id, task_queue->size);
    ...
    while (true) {
        ...
        while (task_queue_is_empty(task_queue) && !termination) {
            ...
            fprintf(stdout, "Worker %d waiting for new tasks\n",
                    process_id);
        ...
        fprintf(stdout, "Worker %d got new tasks\n",
                process_id);
        pthread_mutex_unlock(&mutex);
    }
...
void *receiver_start() {
    ...
    while (!termination) {
        ...
        pthread_mutex_lock(&mutex);
        fprintf(stdout, "Receiver %d waiting\n", process_id);
        while (!task_queue_is_empty(task_queue)) {
            pthread_cond_wait(&receiver_cond, &mutex);
        }
        fprintf(stdout, "Receiver %d sends requests\n", process_id);
        pthread_mutex_unlock(&mutex);
        ...
        for (int i = 0; i < process_count; ++i) {
            ...
            if (task.id != EMPTY_QUEUE_RESPONSE) {
                fprintf(stdout, "Receiver %d adds task"
                                "[ID: %d, pID: %d, W: %d]\n",
                                process_id, task.id, task.process_id, task.weight);
                pthread_mutex_lock(&mutex);
                task_queue_push(task_queue, task);
        ...
        if (received_tasks == 0) {
            pthread_mutex_lock(&mutex);
            termination = true;
            fprintf(stdout, "Receiver %d terminating process\n",
                    process_id);
            pthread_mutex_unlock(&mutex);
        }

        pthread_mutex_lock(&mutex);
        fprintf(stdout, "Receiver %d received %d tasks\n",
                process_id, received_tasks);
...
void *sender_start() {
    while (true) {
        ...
        pthread_mutex_lock(&mutex);
        fprintf(stdout, "Sender %d waiting\n", process_id);
        pthread_mutex_unlock(&mutex);
        ...
        fprintf(stdout, "Sender %d sends task"
                        "[ID: %d, pID: %d, W: %d]\n",
                        process_id, task.id, task.process_id, task.weight);
        pthread_mutex_unlock(&mutex);
